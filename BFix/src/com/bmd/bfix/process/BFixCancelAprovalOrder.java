...
public class BFixCancelAprovalOrder implements Runnable {
	private BFixApplication bFixAplication;
	private BFixOrderTableModel orderTableModel;
	private BFixOrder order;
	
	public BFixCancelAprovalOrder(BFixOrder order, BFixApplication bFixAplication, BFixOrderTableModel orderTableModel) {
	...
	public void run() {
			...
			try {
				...
				if (conn == null) {
					System.out.println("Cancel Aproval Order Db Bağlantı Hatası! (1101)");
				} else {
					cs = conn.prepareCall("{? = call uop14.senkron_kanal.emir_iptal_onayi_bfix(?,?,?,?,?,?,?)}");
					...
					if(result==0){
						...
						System.out.println("Cancel Aproval Order yeni emir onayı hatası emir sira no :" + order.getClOrdID() + " sonuc : " + result);
						
							
				}
		} catch (Exception e) {
			System.out.println("Cancel Aproval Order ExecuteQuery Hatası! (1102)");
			e.printStackTrace();
		} finally {
			try {
				if (!conn.isClosed())
