...
public class BFixExecutionTable extends JTable implements MouseListener {
	private transient BFixApplication application;

	public BFixExecutionTable(BFixExecutionTableModel bFixExecutionTableModel) {
		super(bFixExecutionTableModel);
	...
	public Component prepareRenderer(TableCellRenderer renderer, int row,
			int column) {
		BFixOrder order = (BFixOrder) ((BFixExecutionTableModel) dataModel)
				.getOrder(row);
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getButton() == MouseEvent.BUTTON3) {
			...
			JOptionPane.showMessageDialog(null,
					mespars.FixMessageParseByString(order.getMessage()));
